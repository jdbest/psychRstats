...
# Install new packages if they are not yet installed
packages <- c("HistData", "palmerpenguins")
...
We'll continue using data from the `penguins` data set you were introduced to in Lab 2. It should have been downloaded when you ran this document! If you run into errors, consult with the professor or course assistant (or just installed the packages yourself).
...
*Note*: If you're not seeing a place to enter text and the buttons "run code" and "submit answer" here, you're missing a package! Copy this code---`remotes::install_github("rstudio-education/gradethis")`---and then close this document and run it in the console. You might also need to restart RStudio. If you've edited this document at all, that could be an issue, too... you can always try downloading it again. Any errors? Ask a course assistant or the professor for help. If you get an error that the `penguins` data doesn't exist (e.g., <span style="color:red;">Error: object 'penguins' not found</span>), you should also check in!
...
In class, we've been spending a lot of time discussing frequency tables. Fortunately, you don't *usually* have to do those by hand. There are two ways of doing this in R. One is called `table()` and one is called `count()` and comes from the `dplyr` package. 
...
As prof of business analytics Di Cook notes, the function we used to install these, `install.packages("packagename")`, is like putting in a light bulb. Running `library(packagename)` is how you turn on the lights. You have to do it whenever you want to use them in a document---and then they're on for the document. Here, our `packagename` is `ggplot2`---let's turn it on. 
...
Ultimately, I think that histograms aren't that useful---there are better plots for most things you might need, including checking on whether data is normally distributed. But they're good to know, anyway!
...
Why do you think it's not accurate? Discuss in your groups, if possible. (Note the lines across the bars?)
...
This is where the tutorial ends, for today.

To let me know any questions you might have, give feedback, or just comment on the material [fill out the survey](https://bit.ly/psy203exit). Select Lab 3. 

Then, go back to the folder where this was downloaded and open `visualizing-data-exercises.Rmd` for today's exercises. 