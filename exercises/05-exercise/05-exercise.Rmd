---
title: "Refreshing and Reviewing: Exercises"
author: ""
...
5. Create a new variable called `w` (a new one) which is assigned to 5 values that you make up, combined with `c()`. Then find the mean and standard deviation of the values, using that variable.
...
```{r}
# DATA$haircolor <- factor(DATA$haircolor,
#                              levels = c(1, 2),
...
So, we can define our comparison distribution as follows: it is a *z*-distribution based on the **sampling distribution of the mean**, which has a mean of the mean defined by `follow.mean` above and a standard deviation equal to the *standard error of the mean*, saved as `follow.sem`. 
...
Okay, our *z*-score here is what you've saved as `follow.z`.
...
What do we need to do to find an estimate of the population standard deviation? The following things, none of which are actually knew. (If you can't see them, mouse over the symbols between $ signs to see the formulas properly formatted, if you're viewing this as an Rmd file.)
...
It's the *t*-distribution with *df* of 66, a mean of 500, and a standard deviation (the standard error of the mean) of what you've defined as `s.m`. Your `s.m` should be 53.64 (if you round it). 
...
Okay, so we can say that $t=$ what you've defined as `t.stat`. Is it 3.46 (rounded)? It should be.
